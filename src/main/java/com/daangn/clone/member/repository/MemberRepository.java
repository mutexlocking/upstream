...
public interface MemberRepository extends JpaRepository<Member, Long> {
    ...
    @Query("select distinct m from Member m join fetch m.memberChattingList mc join fetch mc.chattingRoom c where m.username = :username order by c.createdAt desc")
    Member findMemberWithChatting(@Param("username") String username);

    @Query("select distinct m from Member m join fetch m.memberChattingList where m.username = :username")
